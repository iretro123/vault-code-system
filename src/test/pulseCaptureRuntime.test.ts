...
function png(width=2200,height=1600){
...
function fixture(){
  const chart={boundingBox:vi.fn().mockResolvedValue({width:1100,height:800}),screenshot:vi.fn().mockResolvedValue(png())};
...
describe('durable screenshot processing',()=>{
  ...
  it('rejects a 1x screenshot even when its file size is large enough',async()=>{
    const {env,rpc,chart}=fixture();chart.screenshot.mockResolvedValue(png(1100,800));
    expect(await runCapture(env,rpc)).toBe('retry');
    expect(env.CHART_IMAGES.put).not.toHaveBeenCalled();
    expect(rpc).toHaveBeenLastCalledWith('pulse_spy_capture_finish',expect.objectContaining({p_result:{ok:false,failure:'chart-resolution-unavailable'}}),5000);
  });
  it('retries without saving when the chart never repaints at the required density',async()=>{
    const {env,rpc,page,chart}=fixture();
    page.waitForFunction.mockImplementation(async function wait(fn: () => unknown) { if (fn.name==='chartAtCaptureDensity') throw new Error('timeout'); });
    expect(await runCapture(env,rpc)).toBe('retry');
    expect(chart.screenshot).not.toHaveBeenCalled();
    expect(env.CHART_IMAGES.put).not.toHaveBeenCalled();
    expect(rpc).toHaveBeenLastCalledWith('pulse_spy_capture_finish',expect.objectContaining({p_result:{ok:false,failure:'chart-resolution-unavailable'}}),5000);
