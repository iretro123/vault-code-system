...
const AcademyJournalContent = () => {
  ...
  const handleReset = () => {
    setDate(new Date()); setTicker(""); setWhatHappened(""); setFollowedRules(null); setMistake("none"); setLesson(""); setSaved(false);
  };
    ...
    <Card className="p-6 max-w-lg space-y-5">
      {saved ? (
        <div className="space-y-4 text-center py-4">
          ...
          <Button variant="outline" size="sm" onClick={handleReset}>Log another trade</Button>
        </div>
      ) : (
        <>
          ...
          <div className="space-y-1.5">
            ...
            <Textarea placeholder="Describe the trade in 2-3 sentences..." value={whatHappened} onChange={(e) => setWhatHappened(e.target.value)} rows={3} />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs">Did you follow your rules?</Label>
            <div className="flex gap-2">
              <Button type="button" variant={followedRules === true ? "default" : "outline"} size="sm" onClick={() => setFollowedRules(true)}>Yes</Button>
              <Button type="button" variant={followedRules === false ? "destructive" : "outline"} size="sm" onClick={() => setFollowedRules(false)}>No</Button>
            </div>
          </div>
          <div className="space-y-1.5">
            ...
            <Select value={mistake} onValueChange={setMistake}>
              ...
              <SelectContent>
                {MISTAKES.map((m) => <SelectItem key={m.value} value={m.value}>{m.label}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs">Lesson learned</Label>
            <Input placeholder="One sentence takeaway..." value={lesson} onChange={(e) => setLesson(e.target.value)} />
          </div>
...
export default AcademyJournalContent;
