...
export default function AcademyMessages() {
  ...
  return <section className={`vault-messages ${selected?'has-conversation':''}`} aria-label="Member messages">
    ...
    }}><header><h2 id="vm-new-title">New message</h2><button type="button" className="vm-icon vm-dialog-close" aria-label="Close member search" onPointerDown={event=>{event.preventDefault();setCreating(false);}} onClick={()=>setCreating(false)}><X size={20}/></button></header>
