  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [lessonsRes, journalRes, recapsRes] = await Promise.all([
        supabase.from("lesson_progress").select("id", { count: "exact", head: true }).eq("user_id", user!.id).eq("completed", true).gte("completed_at", startISO).lte("completed_at", endISO),
        (supabase as any).from("journal_entries").select("biggest_mistake").eq("user_id", user!.id).gte("entry_date", startDate).lte("entry_date", endDate),
        supabase.from("academy_messages").select("id", { count: "exact", head: true }).eq("user_id", user!.id).eq("room_slug", "trade-recaps").gte("created_at", startISO).lte("created_at", endISO),
      ]);
...
export default AcademyProgressContent;
