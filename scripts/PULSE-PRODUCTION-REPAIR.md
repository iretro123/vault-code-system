...
**Screenshots are not yet active or verified end to end.** `capture_enabled=false`, `capture_connected=false`, `CAPTURE_SAVE_LOGIN=false`, and no capture-session binding is provisioned. No original SPY image has been attached by this service. The member UI correctly shows capture unavailable.
