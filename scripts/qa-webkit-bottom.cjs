...
const out = '/tmp/vault-webkit-bottom-qa';
...
for(const [device,width,height] of sizes){
 ...
 for(const route of routes){
   ...
   const bottom=await page.evaluate(()=>{const containers=[...document.querySelectorAll('*')].filter(e=>e.scrollHeight>e.clientHeight+5&&['auto','scroll'].includes(getComputedStyle(e).overflowY));containers.forEach(e=>e.scrollTop=e.scrollHeight);window.scrollTo(0,document.body.scrollHeight);return containers.map(e=>({tag:e.tagName,classes:e.className,top:e.scrollTop,max:e.scrollHeight-e.clientHeight}));});
   await page.waitForTimeout(100);
   await page.screenshot({path:path.join(out,name+'-bottom.png'),fullPage:false});
   results.push({device,width,height,route,...details,errors,bottom});page.off('pageerror',listener);
