...
## Extra proof shots if you want a stronger recording

- Briefly show `Settings -> Account` before opening the delete panel so the path is obvious.
- If the app lands back on the welcome/auth screen after deletion, leave it visible for a second before trying the failed sign-in.
- Keep taps deliberate and slow enough for Apple to follow without narration.
