...
USING (bucket_id = 'ticket-screenshots' AND ((storage.foldername(name))[1] = auth.uid()::text OR has_role(auth.uid(), 'operator'::app_role)));
...
ON storage.objects FOR SELECT TO authenticated
USING (bucket_id = 'trade-screenshots' AND ((storage.foldername(name))[1] = auth.uid()::text OR has_role(auth.uid(), 'operator'::app_role)));
