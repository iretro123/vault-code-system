...
BEGIN
  ...
  -- ── Insight Text (highest-signal first) ──
  IF _block_rate > 0.35 THEN
    _insight := 'Many intents are being blocked. Reduce size or trades.';
  ELSIF _red >= 1 THEN
    _insight := 'RED days are the main leak. Protect the downside first.';
  ELSIF _avg_tpd > 3 THEN
    _insight := 'You perform best with fewer trades.';
  ELSIF _most_common IS NOT NULL THEN
    _insight := 'Most blocks come from: ' || _most_common || '.';
  ELSE
    _insight := 'Stability is improving. Keep rules tight.';
  END IF;
  ...
  IF _period = 'weekly' THEN
    INSERT INTO weekly_report (
      ...
      most_common_block_reason, stability_score, mode_fit, insight_text
    ) VALUES (
      ...
      _most_common, _stability, _mode_fit, _insight
    )
    ON CONFLICT (user_id, period_start) DO UPDATE SET
      period_end = EXCLUDED.period_end, days_traded = EXCLUDED.days_traded,
      green_days = EXCLUDED.green_days, yellow_days = EXCLUDED.yellow_days, red_days = EXCLUDED.red_days,
      trades_taken = EXCLUDED.trades_taken, trades_blocked = EXCLUDED.trades_blocked,
      risk_used = EXCLUDED.risk_used, risk_saved = EXCLUDED.risk_saved,
      avg_trades_per_day = EXCLUDED.avg_trades_per_day, avg_risk_per_trade = EXCLUDED.avg_risk_per_trade,
      block_rate = EXCLUDED.block_rate, most_common_block_reason = EXCLUDED.most_common_block_reason,
      stability_score = EXCLUDED.stability_score, mode_fit = EXCLUDED.mode_fit, insight_text = EXCLUDED.insight_text;
  ELSE
