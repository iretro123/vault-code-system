...
-- Fix all 4 notification triggers: remove "AND read_at IS NULL" filter
-- and reset read_at to NULL on update so the card re-appears as unread
  ...
  LOOP
    ...
    ELSE
      ...
      VALUES (_op.user_id, 'coach_reply', _sender_name || ' sent a message',
        LEFT(NEW.body, 120), '/academy/admin/panel?tab=dms', NEW.sender_id);
    END IF;
  END LOOP;
  RETURN NEW;
END;
...
BEGIN
  IF NEW.is_admin = true THEN
    ...
    SELECT id INTO _existing_id FROM public.inbox_items
    WHERE user_id = _ticket_user_id AND sender_id = NEW.user_id AND type = 'coach_reply'
    ORDER BY created_at DESC LIMIT 1;
    ...
    END IF;
  ELSE
    SELECT question INTO _question FROM public.coach_tickets WHERE id = NEW.ticket_id;

    FOR _op IN SELECT user_id FROM public.user_roles WHERE role = 'operator'
    LOOP
      SELECT id INTO _existing_id FROM public.inbox_items
      WHERE user_id = _op.user_id AND sender_id = NEW.user_id AND type = 'coach_reply'
      ORDER BY created_at DESC LIMIT 1;
