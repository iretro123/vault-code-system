...
-- RPC: Close an open trade with result, update vault_state accordingly
CREATE OR REPLACE FUNCTION public.close_trade_intent(
  _user_id UUID,
  _trade_result NUMERIC
)
RETURNS TABLE(
  success BOOLEAN,
  message TEXT,
  new_vault_status TEXT
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Find the open intent to close it
  ...
  _new_status := _state.vault_status;
  ...
  -- Status escalation (no overrides, no confirmations)
  IF _new_risk_remaining <= 0 THEN
    _new_status := 'RED';
    _new_risk_remaining := 0;
  ...
  RETURN QUERY SELECT true,
    CASE
      ...
      WHEN _new_status = 'YELLOW' AND _state.vault_status != 'YELLOW' THEN 'Trade closed. Loss streak detected — vault is now YELLOW.'
