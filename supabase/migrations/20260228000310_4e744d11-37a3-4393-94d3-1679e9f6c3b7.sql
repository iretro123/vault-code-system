...
-- admin_override_access RPC: server-side validated access override with audit logging
...
BEGIN
  ...
  IF NOT (has_role(caller_id, 'operator') OR has_role(caller_id, 'vault_os_owner')) THEN
    RAISE EXCEPTION 'Unauthorized: requires operator or vault_os_owner role';
  END IF;

  -- Validate new_status
  IF new_status NOT IN ('active', 'past_due', 'canceled') THEN
    RAISE EXCEPTION 'Invalid status: must be active, past_due, or canceled';
  END IF;
  ...
  IF NOT FOUND THEN
    ...
    VALUES (target_student_id, 'vault_academy', 'elite_v1', new_status,
            CASE WHEN new_status = 'active' THEN now_ts ELSE NULL END,
            now_ts, now_ts);
  ...
  ELSE
    ...
    SET status = new_status,
        ...
        access_ended_at = CASE WHEN new_status IN ('canceled') THEN now_ts ELSE NULL END,
